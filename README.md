...
- 新增 `codex-provider-switch-repair`：切换模型 provider 后，已有会话仍会使用它们创建时记录的 provider 与模型，因此可能整体失联；该技能先只读诊断 provider 配置、登录状态、模型目录覆盖范围与日志中的鉴权／模型缺失记录，再在确认后备份并修复配置，不改写任何会话历史或转录。
